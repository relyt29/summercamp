...
//    }
    ...
    function decode(bytes memory source) internal pure returns (bytes memory out){
        ...
        while(i<len){
            ...
            if(c2 == -1)
                break;
            out[o++] = bytes1(uint8((c1 * 2 ** 2) | ((c2 & 0x30) / 2 ** 4)));

            do {
                c3 = int8(uint8(source[i++])) & int8(uint8(0xff));
                if(c3 == 61)
                    return out;
            ...
            out[o++] = bytes1(uint8(((c2 & 0xF) * 2 ** 4) | ((c3 & 0x3C) / 2 ** 2)));

            do {
                c4 = int8(uint8(source[i++])) & int8(uint8(0xff));
                if(c4 == 61)
                    return out;
            ...
            out[o++] = bytes1(uint8(((c3 & 0x03) << 6) | c4));
        }
        return out;
    }
}
